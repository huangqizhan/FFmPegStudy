...
int video_sample_filter()
{
    ...
    if ((ret = open_input_file("video.mp4")) < 0)
        goto end;
    if ((ret = init_filters(filter_descr)) < 0)
        goto end;

#if ENABLE_YUVFILE
    FILE *fp_yuv=fopen("test123.yuv","wb+");
        ...
        if (packet.stream_index == video_stream_index) {
            ...
            if (got_frame) {
                ...
                while (1) {
                    ...
                    if (ret < 0)
                        break;
                    printf("Process 1 frame ret = %d \n",ret);
                    printf("format = %d \n",pFrame_out->format);
                    if (pFrame_out->format==AV_PIX_FMT_YUVJ420P) {
#if ENABLE_YUVFILE
                        //Y, U, V
