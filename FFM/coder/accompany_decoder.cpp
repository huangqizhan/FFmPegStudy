...
int AccompanyDecoder::init(const char* audioFile) {
    printf("enter AccompanyDecoder::init");
    audioBuffer = NULL;
    position = -1.0f;
    audioBufferCursor = 0;
    audioBufferSize = 0;
    ...
    // 打开输入文件
    printf("open accompany file %s....", audioFile);
    ...
    if (result != 0) {
        printf("can't open file %s result is %d", audioFile, result);
        return -1;
    } else {
        printf("open file %s success and result is %d", audioFile, result);
    }
    avFormatContext->max_analyze_duration = 50000;
    AVCodecID Id = avFormatContext->audio_codec_id;
    printf("id = %u",Id);
    //检查在文件中的流的信息
    result = avformat_find_stream_info(avFormatContext, NULL);
    if (result < 0) {
        printf("fail avformat_find_stream_info result is %d", result);
        return -1;
    } else {
        printf("sucess avformat_find_stream_info result is %d", result);
    }
    stream_index = av_find_best_stream(avFormatContext, AVMEDIA_TYPE_AUDIO, -1, -1, NULL, 0);
    printf("stream_index is %d", stream_index);
    // 没有音频
    if (stream_index == -1) {
    ...
    // 根据解码器上下文找到解码器
    printf("avCodecContext->codec_id is %d AV_CODEC_ID_AAC is %d", avCodecContext->codec_id, AV_CODEC_ID_AAC);
    ...
    printf("channels is %d sampleRate is %d", avCodecContext->channels, avCodecContext->sample_rate);
//    pAudioFrame = avcodec_alloc_frame();
//    printf("leave AccompanyDecoder::init");
    return 1;
}
...
int AccompanyDecoder::readFrame() {
    printf("enter AccompanyDecoder::readFrame");
    if(seek_req){
        this->seek_frame();
    }
    int ret = 1;
    ...
    //                LOGI(" avcodec_decode_audio4 ...", duration);
                ...
                if (gotframe) {
                    ...
                    if (!seek_success_read_frame_success) {
                        printf("position is %.6f", position);
...
void AccompanyDecoder::destroy() {
    ...
    if (NULL != avFormatContext) {
        printf("leave LiveReceiver::destory");
